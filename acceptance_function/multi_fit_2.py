...
def chi_err(amps, centres, f):
    N_expected = np.max(f(centres), 0) # counts must be positive
    chi = np.sum((amps - N_expected)**2 / N_expected)
    # print(chi, np.min(amps), np.min(f(centres)))
...
def single_fit(data_arr, f=None):
    ...
    if f is None:
        ...
        p_value = chi_err(amps, centres, np.poly1d(p_opt))
    else:
...
def acceptance(plot=False):
    ...
    for i in range(len(q2_ranges)):
        ...
        overall_p_opt, overall_p_value = single_fit(current_bin["costhetal"])
        ...
        for j in range(N_dctk):
            ...
            print(f"q^2 bin: {i}\tctk bin: {j}\tchi^2 p-value: {p_value}")
            
            # Find the fit function
            poly_func = np.poly1d(overall_p_opt)
            ...
            if plot:
                ...
                plt.annotate(f'chi^2 p-value: {p_value:.4f}', (0.05, 0.95), xycoords='axes fraction')
